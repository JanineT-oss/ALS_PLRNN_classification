...
FeatureSets.F1 = ["FC_"];                                                  % feature set ("VOL_", "FC_", "D_")                                              

for repe = 1 : 5
    
    %% paths
    pati='/###/';                                                          % add path containing excel "ALS_ALL_DATA_ALL_FC.xlsx"                                                           
    dum = num2str(repe);
    pato = strcat('/###/FC/FC_classification_', dum , '/');                % add output path for a new classifier OR if you want to reproduce classifier reported in the publication, point path to the respective classifier
    ...
    if repro == 1
        ...
        file = 'RF_classifier_FC.mat'; filename=[pato file]; load(filename); 
        Seed = ClassifierSVM.RandomSeed;
        clear Classifier

        dum = num2str(repe);
        patoR = strcat('/###/FC_replication/FC_classification_repl_', dum , '/'); % add output path you want to store the repl classifier in                             
    ...
    %% load data 
    file_T = 'ALS_ALL_DATA_ALL_FC.xlsx';
    ...
    %Classifier
    if repro == 0
        filename = [pato 'RF_classifier_FC.mat'];
        save(filename, 'ClassifierRF', 'ClassifierSVM');
    elseif repro == 1
        filename = [patoR 'RF_classifier_FC.mat'];
        save(filename, 'ClassifierRF', 'ClassifierSVM');
    end
