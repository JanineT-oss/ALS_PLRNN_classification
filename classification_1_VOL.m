...
FeatureSets.F1 = ["VOL_"];                                                 % feature set ("VOL_", "FC_", "D_")                                              

for repe = 1 : 5
    
    %% paths
    pati='/###/';                                                          % add folder containing excel "ALS_ALL_DATA_ALL.xlsx"                                                           
    dum = num2str(repe);
    pato = strcat('/###/VOL/VOL_classification_', dum , '/');              % add output path for a new classifier OR if you want to reproduce classifier reported in the publication, point path to the respective classifier
    ...
    if repro == 1
        ...
        file = 'RF_classifier_VOL.mat'; filename=[pato file]; load(filename); 
        Seed = ClassifierSVM.RandomSeedNumb;
        clear Classifier

        dum = num2str(repe);
        patoR = strcat('/###/VOL_replication/VOL_classification_repl_', dum , '/'); % add output path you want to store the replicated classifier in                             
    ...
    k=strfind(labels,'VOL_'); idx=(cellfun('isempty', k));                                                                                      
    ...
    %Classifier
    if repro == 0
        filename = [pato 'RF_classifier_VOL.mat'];
        save(filename, 'ClassifierRF', 'ClassifierSVM');
    elseif repro == 1
        filename = [patoR 'RF_classifier_VOL.mat'];
        save(filename, 'ClassifierRF', 'ClassifierSVM');
    end
