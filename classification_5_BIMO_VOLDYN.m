...
for repe = 1 : 5
    ...
    %% paths
    pati='/###/';                                                          % add path containing excel "ALS_ALL_DATA_ALL_VOLDYN.xlsx"                                                                     
    dum = num2str(repe);
    pato = strcat('/###/Bimodal_VOLDYN/VOLDYN_classification_', dum , '/');% add output path for a new classifier OR if you want to reproduce classifier reported in the publication, point path to the respective classifier
    ...
    if repro == 1
        ...
        file = 'RF_classifier_2FOLD_VOLDYN.mat'; filename=[pato file]; load(filename); 
        Seed = ClassifierRF.RandomSeed;
        clear Classifier

        dum = num2str(repe);
        patoR = strcat('/###/Bimodal_VOLDYN_repli/VOLDYN_', dum , '/');    % add output path you want to store the replicated classifier in
    ...
    %% load data 
    file_T = 'ALS_ALL_DATA_ALL_VOLDYN.xlsx';
    ...
    %Classifier
    if repro == 0
        filename = [pato 'RF_classifier_2FOLD_VOLDYN.mat'];
        save(filename, 'ClassifierRF', 'ClassifierSVM');
    elseif repro == 1
        filename = [patoR 'RF_classifier_2FOLD_VOLDYN.mat'];
        save(filename, 'ClassifierRF', 'ClassifierSVM');
    end
