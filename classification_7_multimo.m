...
for repe = 1 : 5
    ...
    dum = num2str(repe);
    pato = strcat('/###/Multim_resScores_rep/Multimodal_classification_', dum , '/');% add output path for a new classifier OR if you want to reproduce classifier reported in the publication, point path to the respective classifier
    ...
    if repro == 1
        ...
        file = 'RF_classifier_Multimodal.mat'; filename=[pato file]; load(filename); 
        Seed = ClassifierRF.RandomSeed;
        clear Classifier

        dum = num2str(repe);
        patoR = strcat('/###/Multimodal_repli/Multimodal_', dum , '/');    % add output path you want to store the replicated classifier in
    ...
    %Classifier
    if repro == 0
        filename = [pato 'RF_classifier_Multimodal.mat'];
        save(filename, 'ClassifierRF', 'ClassifierSVM');
    elseif repro == 1
        filename = [patoR 'RF_classifier_Multimodal.mat'];
        save(filename, 'ClassifierRF', 'ClassifierSVM');
    end
